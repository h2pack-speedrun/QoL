...
table.insert(hook_fns, function()
    modutil.mod.Path.Wrap("OpenRunClearScreen", function(base)
        if config.VictoryScreen and lib.isEnabled(config, public.definition.modpack) then
            thread(function()
                wait(0.5)
    ...
    modutil.mod.Path.Wrap("TraitTrayScreenRemoveItems", function(base, screen)
        if not config.VictoryScreen or not lib.isEnabled(config, public.definition.modpack) then
            return base(screen)
        end
